...
When invoked from `/autopilot`, this skill runs after `/test` and before `/ship` in the pipeline:

`/next` → `/test` → `/harden fix` → `/ship`

In this context:
- Always run in `fix` mode (not audit)
- Focus on the **changed files only** — use `git diff --name-only` to scope the audit to files touched by `/next`
- Skip stack detection (already known from earlier in the loop)
- Be fast — only flag critical and high severity issues, defer medium/low
