---
name: checkup
description: Git repo health checkup and cleanup — removes stale worktrees/branches, prunes remotes, runs gc, and flags repo hygiene issues. Use "now" to skip confirmation.
user-invocable: true
...
Report what was cleaned using the standard output format:
...
═══════════════════════════════════════
  ...
  Done:
    ...
    ✓ Deleted N local branches, N remote branches
    ✓ Ran git gc — reclaimed Xmb
    • No issues needing manual attention

  Next: repo is clean — ready for work

═══════════════════════════════════════
```

If there are issues needing attention, use status `Needs Work` and list them in the Next section.
