...
Turn a rough, informal description into a well-structured GitHub issue. The user describes a problem or feature in whatever words they want — this skill investigates the codebase, fills in technical context, and creates a detailed issue ready for implementation.
...
- Flag if this should be broken into multiple issues
...
- **Large** — spans multiple modules, needs design decisions, may need to be broken into sub-issues

If the scope is **large**, ask the user if they want one big issue or multiple smaller ones. If they want smaller ones, create each one individually (step 4) and note the dependencies between them.
...
[If large and broken into sub-issues: "Part of #N, #M, #O"]
```
...
If creating multiple sub-issues for a large scope, create them in dependency order and cross-reference them in each body.
