...
| `ui` | Open it in a browser, click around, look at it. **This includes platform integrations** — if the app is a Discord bot, Slack bot, browser extension, etc., the platform IS the UI. Open Discord/Slack/etc. in Playwright and test from the user's perspective. |
...
### 5a. Pre-flight check (DO THIS FIRST — before any testing)

**This step must complete before you run a single test.** Do not start CLI tests, API tests, or anything else until pre-flight passes. Tell the user "Checking for blockers before starting..." then verify:

1. **todo.txt** — any incomplete setup steps? Do them or tell the user.
2. **Environment** — missing env vars, config files, API keys the app needs? Check `.env`, `config.*`, README setup instructions.
3. **Playwright MCP** — is it available? Try loading `mcp__playwright__browser_navigate` via ToolSearch. If unavailable, warn the user that visual tests will be skipped.
4. **Auth/sessions** — will testing require logging into third-party services (Discord, GitHub, Slack, etc.)? If so, open the service in Playwright and check if there's an active session. If not, tell the user: "I need you to log in to [service] in the browser window I just opened, then tell me when you're done."
5. **Dev server** — can it start? Are dependencies installed? Run a quick check (`node_modules` exists, venv exists, etc.)

**If blockers are found:** Report them all at once and wait for the user to resolve them. Do NOT start testing with known blockers.

**If no blockers:** Say so and start immediately.

### 5b. Setup

1. Start the dev server in background via Bash (`run_in_background: true`)
2. Wait for it to be ready, then health-check the URL
...
- Use `mcp__playwright__browser_snapshot` to verify content, structure, and text. **Snapshots are your primary verification tool** — they tell you what's on the page, what the bot said, what elements exist. Use them to check functional correctness.
- Interact: click buttons, fill forms, navigate — whatever a real user would do
- After interactions, snapshot again to verify the response/result
...
**When to screenshot vs. when to snapshot:**
- **Snapshot** = verify what things **say** (text content, structure, element presence, bot responses). Fast, always use this first.
- **Screenshot** = verify how things **look** (layout, styling, alignment, colors, rendering). Only take screenshots when visual appearance matters — landing pages, dashboards, UI components, responsive layouts. Don't screenshot just to confirm a text response you can already read in the snapshot.

**DB/state tests:**
- Trigger the action, then verify the side effect

**Visual verification guidelines:**
- Screenshot each unique page/screen **once** when you first visit it — to verify layout and styling
- Don't re-screenshot the same page after every interaction unless the visual layout changed significantly (e.g., modal appeared, theme toggled, viewport resized)
...
### 5d. Responsive and theme sweep (UI projects only)
...
Wrap the entire results report in the standard box format:

```
═══════════════════════════════════════
  QA — Complete (or Needs Work)
═══════════════════════════════════════

  Total: XX | Passed: XX | Failed: XX | Skipped: XX

  [Results table, failures, visual issues as subsections]

  Next: /ship — everything works
  (or)
  Next: fix N failures, then re-run /qa auto

═══════════════════════════════════════
```

Use status `Complete` when verdict is "Ship it", `Needs Work` when verdict is "Needs work" or "Partial".
