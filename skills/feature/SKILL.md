...
name: feature
description: >
  Plan and scope multi-issue feature work. Analyzes the repo's architecture, conventions,
  and patterns, then breaks features into phased issues with a roadmap.
  For single issues (bugs, chores, small enhancements), use /issue instead.
user-invocable: true
argument-hint: >
...
Plan and scope multi-issue feature work for an existing project. Analyzes the repo to understand how code is written, structured, tested, and shipped — then breaks features into phased issues that fit naturally into the codebase.

**Scope guard:** This skill is for work that needs multiple issues and/or phased planning. If the user's description is a single bug, chore, or small self-contained change, tell them:

> This looks like a single issue. Use `/issue` instead — it'll scope and create it faster.

Then stop. Do not over-plan simple work.
