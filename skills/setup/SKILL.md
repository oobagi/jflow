...
Present the final summary using the standard output format:
...
═══════════════════════════════════════
  ...
  Files created:
    • README.md, AGENTS.md, ROADMAP.md, LICENSE
    • .github/workflows/ci.yml, roadmap-sync.yml
    • [language-specific files]

  Automation:
    • N GitHub issues created
    • CI: lint + test + build on push/PR
    • Roadmap sync: auto-checkbox on issue close/reopen
    • Branch protection: require CI + PR

  Done:
    ✓ Created repo and pushed
    ✓ Created N issues with full specs
    ✓ Linked roadmap items to issues
    ✓ Set up branch protection
    ✓ Set up ROADMAP_PAT secret

  Next:
    1. Check todo.txt for manual setup steps (API keys, secrets, etc.)
    2. /next to start building
